...
if grep -r "domain.core\." $SRC_DIR/domain/foundation/ --include="*.java"; then
    echo "❌ Foundation에서 Core를 참조하는 위반이 발견되었습니다."
    FAIL_COUNT=$((FAIL_COUNT + 1))
fi

# 2. Foundation → Support 역참조 검사 (금지됨)
if grep -r "domain.support\." $SRC_DIR/domain/foundation/ --include="*.java"; then
    echo "❌ Foundation에서 Support를 참조하는 위반이 발견되었습니다."
    FAIL_COUNT=$((FAIL_COUNT + 1))
fi

# 3. Support → Core 역참조 검사
if grep -r "domain.core\." $SRC_DIR/domain/support/ --include="*.java"; then
    echo "❌ Support에서 Core를 참조하는 위반이 발견되었습니다."
    FAIL_COUNT=$((FAIL_COUNT + 1))
fi

# 4. Core 내부 도메인 간 직접 참조 검사 (자기 자신 제외)
# (예: core/trade에서 core/direct 참조 등)
...
for domain in "${CORE_DOMAINS[@]}"; do
    for target in "${CORE_DOMAINS[@]}"; do
        if [ "$domain" != "$target" ]; then
            if grep -r "domain.core.$target\." $SRC_DIR/domain/core/$domain/ --include="*.java"; then
