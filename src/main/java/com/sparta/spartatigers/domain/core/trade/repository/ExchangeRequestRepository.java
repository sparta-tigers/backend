...
{
    boolean existsBySenderIdAndReceiverIdAndItemId(
        Long senderId,
        Long receiverId,
        Long itemId
    );
    ...
    Page<ExchangeRequest> findAllReceiveRequest(
    ...
    Page<ExchangeRequest> findAllSentRequest(
    ...
    @EntityGraph(attributePaths = { "receiver", "sender", "item" })
    @Query(
        "select e from exchange_request e where e.sender.id = :senderId and e.status = :status"
    )
    Page<ExchangeRequest> findAllSentRequestWithStatus(
        @Param("senderId") Long senderId,
    ...
    @EntityGraph(attributePaths = { "receiver", "sender", "item" })
    @Query(
        "select e from exchange_request e where e.receiver.id = :receiverId and e.status = :status"
    )
    Page<ExchangeRequest> findAllReceiveRequestWithStatus(
        @Param("receiverId") Long receiverId,
        @Param("status") ExchangeStatus status,
        Pageable pageable
    );
    ...
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query(
        "SELECT e FROM exchange_request e WHERE e.item.id = :itemId AND e.status = :status"
    )
    List<ExchangeRequest> findByItemIdAndStatusForUpdate(
    ...
    default ExchangeRequest findExchangeRequestByIdOrElseThrow(
        Long exchangeRequestId
    ) {
        return findByIdAndStatus(
            exchangeRequestId,
    ...
    default ExchangeRequest findAcceptedRequestByIdOrElseThrow(
        Long exchangeRequestId
    ) {
        return findByIdAndStatus(
            exchangeRequestId,
