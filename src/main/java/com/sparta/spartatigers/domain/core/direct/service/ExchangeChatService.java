...
public class ExchangeChatService {
    ...
    public void sendMessage(Long senderId, ChatMessageRequest request) {
        ...
        boolean isLimited = redisRateLimiter.isRateLimited(
            rateLimitKey,
            MESSAGE_LIMIT,
            LIMIT_DURATION
        );
        if (isLimited) {
            log.warn(
                "[sendMessage] 메시지 전송 레이트 리밋 초과 - senderId: {}",
                senderId
            );
        ...
        if (room.isCompleted()) {
            log.warn(
                "[sendMessage] 완료된 채팅방 전송 차단 - roomId: {}, senderId: {}",
                roomId,
                senderId
            );
        ...
        String exchangeStatusStr = tradeQueryDao.getExchangeStatus(
        ...
        switch (exchangeStatusStr) {
            ...
            case "PENDING" -> {
                log.warn(
                    "[sendMessage] 비수락(PENDING) 상태 채팅방 전송 차단 - roomId: {}, senderId: {}",
                    roomId,
                    senderId
                );
            ...
            case "COMPLETED" -> {
                log.warn(
                    "[sendMessage] 완료(COMPLETED) 상태 채팅방 전송 차단 - roomId: {}, senderId: {}",
                    roomId,
                    senderId
                );
                throw new InvalidRequestException(
                    ExceptionCode.EXCHANGE_ALREADY_COMPLETED
                );
            }
            case "REJECTED" -> {
                log.warn(
                    "[sendMessage] 거절(REJECTED) 상태 채팅방 전송 차단 - roomId: {}, senderId: {}",
                    roomId,
                    senderId
                );
                throw new InvalidRequestException(
                    ExceptionCode.EXCHANGE_ALREADY_REJECTED
                );
            }
            default -> {
                log.error(
                    "[sendMessage] 알 수 없는 상태 - roomId: {}, status: {}",
                    roomId,
                    exchangeStatusStr
                );
                throw new InvalidRequestException(
                    ExceptionCode.INVALID_EXCHANGE_STATUS
                );
            }
        }
        ...
        DirectMessage savedMessage = directMessageRepository.save(
            DirectMessage.of(
                room,
                sender,
                messageText,
        ...
        redisPublisher.publish(
            "directRoom:" + roomId,
            RedisMessage.from(savedMessage)
        );
        log.info(
            "[1:1 채팅] PUBLISH / roomId={} , messageId={}",
            roomId,
            savedMessage.getId()
        );
        ...
        // 수신자가 접속중이면 읽음 처리
        boolean receiverOnline = sessionRegistry.isUserInRoom(
            roomId,
            receiverId
        );
        if (receiverOnline) {
            ...
            RedisMessage readStatusMessage = RedisMessage.readStatus(
                savedMessage.getId(),
                roomId,
                true
            );
            ...
            log.info(
                "[1:1 채팅] REPUBLISH / roomId={} , messageId={} , read={}",
                roomId,
                readStatusMessage.getMessageId(),
                readStatusMessage.isRead()
            );
        }
    }
