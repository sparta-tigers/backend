...
public class OcrService {
            ...
            for (AnnotateImageResponse res : responses) {
                if (res.hasError()) {
                    throw new RuntimeException(
                        "OCR 인식 오류 : " + res.getError().getMessage()
                    );
                }
    ...
    public String parseSeatInfo(String ocrText) {
        ...
        Pattern typePattern = Pattern.compile(
            "([가-힣a-zA-Z]+(석|존)(\\s*\\([0-9a-zA-Z가-힣]+\\))?)"
        );

        // 3. 구역/블록/블럭
        Pattern blockPattern = Pattern.compile(
            "([0-9a-zA-Z가-힣]+)\\s*(구역|블록|블럭)"
        );

        // 4. 열 (일, 연, 엘 오인식 )
        // 뒤에 -번, -석이 오는것 전부!
        Pattern rowPattern = Pattern.compile(
            "([0-9a-zA-Z]+)\\s*(열|연|엘|일(?=\\s*[0-9]+\\s*(번|석|th|st|nd|rd)))"
        );

        // 5. 번호
        Pattern seatPattern = Pattern.compile(
            "([0-9]+)\\s*(번|석|th|st|nd|rd)"
        );
        ...
        String block = blockMatcher.find()
            ? blockMatcher.group(1) + blockMatcher.group(2)
            : "";
        String row = rowMatcher.find() ? rowMatcher.group(1) + "열" : "";
        String seat = seatMatcher.find() ? seatMatcher.group(1) + "번" : "";

        // 파싱된 결과를 순서대로 조합
        String parsedSeat = String.format(
            "%s %s %s %s %s",
            zone,
            type,
            block,
