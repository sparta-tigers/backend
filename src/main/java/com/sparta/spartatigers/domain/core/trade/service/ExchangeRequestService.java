...
public class ExchangeRequestService {
    ...
    public Long createExchangeRequest(
        ...
        ExchangeRequest exchangeRequest = ExchangeRequest.of(
            item,
            sender,
            receiver,
            have
        ...
        log.info(
            "[FCM 교환요청 알림] receiverId={}, senderId={}",
            receiver.getId(),
            sender.getId()
        );
    ...
    public Page<ReceiveRequestResponseDto> findAllReceiveRequest(
        ...
        Page<ExchangeRequest> exchangeRequestList =
            exchangeRequestRepository.findAllReceiveRequest(
                user.getId(),
                pageable
            );
    ...
    public Page<SendRequestResponseDto> findAllSendRequest(
        ...
        User user = getUser(tokenClaim.getUserId());
        Page<ExchangeRequest> exchangeRequestList =
            exchangeRequestRepository.findAllSentRequest(
                user.getId(),
                pageable
            );
    ...
    @Transactional
    public ExchangeRoomResponseDto updateRequestStatus(
        ...
        ExchangeRequest exchangeRequest = exchangeRequestRepository
            .findByIdForUpdate(exchangeRequestId)
            .orElseThrow(() ->
                new ServerException(ExceptionCode.EXCHANGE_REQUEST_NOT_FOUND)
            );
        ...
        if (exchangeRequest.getStatus() == ExchangeStatus.ACCEPTED) {
            ...
            rejectOtherPendingRequests(
                exchangeRequest.getItem(),
                exchangeRequest.getId()
            );

            // [FIX] 수락 시점에 명시적으로 채팅방 생성 대신 이벤트 발행
            applicationEventPublisher.publishEvent(
                new TradeAcceptedEvent(
                    exchangeRequestId,
            ...
            sendNotification(
                exchangeRequest.getSender().getFcmToken(),
                "교환 요청 수락",
                String.format(
                    "'%s'에 대한 교환 요청이 수락되었어요.",
    ...
    private void rejectOtherPendingRequests(Item item, Long acceptedRequestId) {
        ...
        List<ExchangeRequest> pendingRequests =
            exchangeRequestRepository.findByItemIdAndStatusForUpdate(
                item.getId(),
    ...
    public void completeExchange(
        ...
        ExchangeRequest exchangeRequest =
            exchangeRequestRepository.findAcceptedRequestByIdOrElseThrow(
                exchangeRequestId
            );
        exchangeRequest.validateReceiverIsOwner(user);

        Item item = itemRepository
            .findById(exchangeRequest.getItem().getId())
            .orElseThrow(() ->
        ...
        ItemLocationUpdatedEvent event = new ItemLocationUpdatedEvent(
            item.getUser().getId(),
            "REMOVE_ITEM",
    ...
    @Transactional(readOnly = true)
    public Page<ReceiveRequestResponseDto> findMyExchangeRequests(
        String role,
        ...
        if ("sender".equals(role)) {
            if (status == null) {
                requests = exchangeRequestRepository.findAllSentRequest(
                    userId,
                    pageable
                );
            } else {
                requests =
                    exchangeRequestRepository.findAllSentRequestWithStatus(
                        userId,
                        status,
                        pageable
                    );
            ...
            if (status == null) {
                requests = exchangeRequestRepository.findAllReceiveRequest(
                    userId,
                    pageable
                );
            } else {
                requests =
                    exchangeRequestRepository.findAllReceiveRequestWithStatus(
                        userId,
                        status,
                        pageable
                    );
    ...
    private Page<ReceiveRequestResponseDto> mapToReceiveResponse(
        Page<ExchangeRequest> requests
    ) {
        Map<Long, Long> roomIdMap = getRoomIdMap(requests);
        return requests.map(req ->
            ReceiveRequestResponseDto.from(req, roomIdMap.get(req.getId()))
        );
    }

    private Page<SendRequestResponseDto> mapToSendResponse(
        Page<ExchangeRequest> requests
    ) {
        Map<Long, Long> roomIdMap = getRoomIdMap(requests);
        return requests.map(req ->
            SendRequestResponseDto.from(req, roomIdMap.get(req.getId()))
        );
    }
    ...
    private void checkDuplicateExchangeRequest(
        Long senderId,
        Long receiverId,
        Long itemId
    ) {
        boolean isExisted =
            exchangeRequestRepository.existsBySenderIdAndReceiverIdAndItemId(
                senderId,
                receiverId,
                itemId
            );
        ...
        if (isExisted) {
            throw new InvalidRequestException(
                ExceptionCode.EXCHANGE_REQUEST_DUPLICATED
            );
        }
    }
