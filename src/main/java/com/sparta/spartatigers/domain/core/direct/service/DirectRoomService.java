...
public class DirectRoomService {
    ...
    @Transactional
    public DirectRoomCreateResponseDto createRoom(
        ...
        ExchangeUsersInfo usersInfo = tradeQueryDao.getExchangeUsers(
            exchangeRequestId
        );

        // 권한 확인: 요청한 사람이 교환 요청의 sender 또는 receiver여야 함
        if (
            !usersInfo.senderId().equals(currentUserId) &&
        ...
        DirectRoom room = directRoomRepository
            ...
            .orElseGet(() ->
                directRoomRepository.save(
                    DirectRoom.create(exchangeRequestId, sender, receiver)
                )
            );
    ...
    public Page<DirectRoomResponseDto> getRoomsForUser(
        Long currentUserId,
        Pageable pageable
    ) {
        log.info(
            "[getRoomsForUser] 채팅방 목록 조회 - 사용자 ID: {}",
            currentUserId
        );
        Page<DirectRoom> rooms =
            directRoomRepository.findBySenderIdOrReceiverIdWithUsersAndItem(
                currentUserId,
                pageable
            );
        ...
        List<DirectMessageRepository.UnreadCountProjection> unreadCountsResult =
            directMessageRepository.countUnreadMsgInBatch(
                roomIds,
                currentUserId
            );
        ...
        Map<Long, Boolean> onlineStatusesMap =
            userConnectService.getOnlineStatuses(opponentIds);
        ...
        return rooms.map(room -> {
            ...
            boolean isOnline = onlineStatusesMap.getOrDefault(
                opponentId,
                false
            );
            ...
            TradeItemInfo itemInfo = tradeQueryDao.getTradeItemInfo(
                room.getExchangeRequestId()
            );
            return DirectRoomResponseDto.from(
                room,
                itemInfo,
                room.getExchangeRequestId(),
                unreadCount,
    ...
    @Transactional(readOnly = true)
    public DirectRoomItemResponseDto getRoomItem(
        Long directRoomId,
        Long currentUserId
    ) {
        DirectRoom room = directRoomRepository
            ...
            .orElseThrow(() ->
                new InvalidRequestException(ExceptionCode.CHATROOM_NOT_FOUND)
            );

        TradeItemDetailInfo detailInfo = tradeQueryDao.getTradeItemDetail(
            room.getExchangeRequestId()
        );
        ...
        return DirectRoomItemResponseDto.from(
            ...
            detailInfo.status(),
            detailInfo.ownerId(),
            detailInfo.ownerNickname(),
            detailInfo.exchangeStatus(),
            opponentId,
            opponentNickname
        );
    }
    ...
    public void deleteRoom(Long directRoomId, Long currentUserId) {
        log.info(
            "[deleteRoom] 채팅방 삭제 요청 - roomId: {}, userId: {}",
            directRoomId,
            currentUserId
        );
        ...
        if (!isSender && !isReceiver) {
            log.warn(
                "[deleteRoom] 삭제 권한 없음 - userId: {}, roomId: {}",
                currentUserId,
                directRoomId
            );
            throw new InvalidRequestException(ExceptionCode.FORBIDDEN_REQUEST);
    ...
    public void deleteRoomByExchangeRequestId(Long exchangeRequestId) {
        ...
        directRoomRepository.delete(room);
        log.info(
            "[deleteRoomByExchangeRequestId] 채팅방 삭제 완료 - roomId: {}",
            room.getId()
        );
    }
    ...
    public List<DirectRoomMessageResponse> getMessagesAfter(
        ...
        log.info(
            "[getMessagesAfter] 누락 메시지 조회 - roomId: {}, after: {}, userId: {}",
            roomId,
            afterTimestamp,
            currentUserId
        );
        ...
        return directMessageRepository
            .findMessagesAfterTimestamp(roomId, afterTimestamp, pageable)
            .stream()
