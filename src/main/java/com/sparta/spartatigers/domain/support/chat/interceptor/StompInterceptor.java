...
import com.sparta.spartatigers.domain.foundation.user.auth.service.JwtTokenService;
import com.sparta.spartatigers.domain.support.chat.model.ChatDomainType;
...
public class StompInterceptor implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        ); // stomp 메세지의 헤더를 분석 ( 커멘드, 세션아이디 등등..)
        StompCommand command = accessor.getCommand();

        if (StompCommand.CONNECT.equals(command)) {
            // [DEBUG] CONNECT 시도 감지 로그
            log.info(
                "[StompInterceptor] CONNECT 시도 감지 - sessionId: {}",
            ...
            if (rawToken == null || !rawToken.startsWith("Bearer ")) {
                log.warn(
                    "[StompInterceptor] 토큰 없음 또는 형식 오류 - sessionId: {}, Authorization 헤더: {}",
                    accessor.getSessionId(),
                    rawToken
                );
                throw new IllegalArgumentException(
                    "WebSocket CONNECT 시 Authorization Bearer 토큰이 필요합니다."
                );
            }
            ...
            try {
                ...
                log.warn(
                    "[StompInterceptor] JWT 검증 실패 - sessionId: {}, 원인: {}",
                    accessor.getSessionId(),
                    e.getMessage()
                );
                throw new IllegalArgumentException(
                    "WebSocket CONNECT 토큰 검증에 실패했습니다."
                );
            }
            ...
            User user = userRepository
                ...
                .orElseThrow(() ->
                    new IllegalArgumentException(
                        "WebSocket CONNECT: 유저를 찾을 수 없습니다."
                    )
                );
            ...
            if (domain.equals(ChatDomainType.EXCHANGE)) {
                userSessionRegistry.registerSession(
                    user.getId(),
                    accessor.getSessionId()
                );
            }
    ...
    private ChatDomainType resolveDomain(String domainRaw) {
        if (domainRaw == null || domainRaw.isBlank()) {
            throw new IllegalArgumentException(
                "ChatDomain 헤더가 필수입니다. 클라이언트 STOMP connectHeaders에 ChatDomain을 명시하세요."
            );
        }
        if (
            "liveboard".equalsIgnoreCase(domainRaw)
        ) return ChatDomainType.LIVEBOARD;
        if (
            "directroom".equalsIgnoreCase(domainRaw)
        ) return ChatDomainType.EXCHANGE;
        if (
            "location".equalsIgnoreCase(domainRaw)
        ) return ChatDomainType.LOCATION;
        throw new IllegalArgumentException(
            "지원하지 않는 ChatDomain: '" +
                domainRaw +
                "'. 허용값: liveboard, directroom, location"
        );
    }
}
