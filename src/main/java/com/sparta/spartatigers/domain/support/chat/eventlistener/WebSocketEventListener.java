package com.sparta.spartatigers.domain.support.chat.eventlistener;
...
public class WebSocketEventListener {
    ...
    public void handleSessionSubscriveEvent(SessionSubscribeEvent event) {
        ...
        if (
            destination != null &&
            destination.startsWith("/server/directRoom/") &&
            userId != null
        ) {
            Long roomId = Long.parseLong(
                destination.substring("/server/directRoom/".length())
            );
            sessionRegistry.registerUserInRoom(roomId, userId);
            log.info(
                "[1:1 채팅] 입장 이벤트 감지 - user {} entered room {}",
                userId,
                roomId
            );
        }
    ...
    public void handleUnsubsribe(SessionUnsubscribeEvent event) {
        ...
        if (
            ...
            log.info(
                "[1:1 채팅] 퇴장 이벤트 감지 - user {} left room {}",
                userId,
                roomId
            );
        }
