...
public class WeatherParser {
    ...
    // ✅ 초단기실황 : 카테고리 - 응답 매핑
    public static Map<String, String> toNcstMap(
        List<OriginResponse.Item> items
    ) {
        Map<String, String> m = new HashMap<>();
        for (OriginResponse.Item it : items) {
            if (it.category != null && it.obsrValue != null) {
                // obsrValue는 실황용 응답값임
    ...
    public static Map<String, String> toClosestFcstMap(
        List<OriginResponse.Item> items,
        java.time.Clock clock
    ) {
        LocalDateTime closest = getClosestTimeToNow(items, clock);
        if (closest == null) return Collections.emptyMap();
        String targetDate = closest.format(
        ...
        for (OriginResponse.Item it : items) {
            if (
                targetDate.equals(it.fcstDate) && targetTime.equals(it.fcstTime)
            ) {
                if (it.category != null && it.fcstValue != null) {
                    // fcstValue는 실황용 응답값임
    ...
    > toFcstMapGroupedByDateTime(List<OriginResponse.Item> items) {
        Map<String, Map<String, Map<String, String>>> dateTimeCategoryMap =
            new LinkedHashMap<>();
     ...
     */
    public static LocalDateTime getClosestTimeToNow(
        List<OriginResponse.Item> items,
        java.time.Clock clock
    ) {
        LocalDateTime threshold = LocalDateTime.now(clock).truncatedTo(
            ChronoUnit.HOURS
        );
        ...
        for (OriginResponse.Item it : items) {
            ...
            if (fcstDateTime.isBefore(threshold)) continue;
    ...
    // 가장 최근 발표 찾기
    public static LocalDateTime latestBaseDateTime(
        List<OriginResponse.Item> items
    ) {
        ...
        for (OriginResponse.Item it : items) {
            ...
            if (
                target != null && (latest == null || target.isAfter(latest))
            ) latest = target;
        }
        return latest;
    }
     ...
     */
    public static LocalDateTime toDateTimeFromFcst(
        String fcstDate,
        String fcstTime
    ) {
