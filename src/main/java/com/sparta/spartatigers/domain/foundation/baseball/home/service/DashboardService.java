...
public class DashboardService {
    ...
    public HomeDashboardResponseDto getDashboardSummary(Long userId) {
        ...
        // 2. 응원 팀 및 관련 정보 조회
        FavoriteTeam favTeam = favTeamRepository
            .findByUserId(userId)
            .orElse(null);
        ...
        if (favTeam != null && favTeam.getTeam() != null) {
            favoriteTeamCode = (favTeam.getTeam().getCode() != null)
                ? favTeam.getTeam().getCode().name()
                : null;
     ...
     */
    private List<LineupBatter> fetchTodayLineup(
        Long teamId,
        LocalDateTime now
    ) {
        ...
        return matchRepository
            .findFirstByMatchTimeBetweenAndTeam(
                startOfDay,
                endOfDay,
                teamId,
                LeagueType.REGULAR
            ...
            .map(match -> {
                ...
                try {
                    ...
                    LineupCacheDto lineupCache = objectMapper.convertValue(
                        cachedData,
                        LineupCacheDto.class
                    );

                    // 응원 팀이 홈인지 어웨이인지에 따라 필터링
                    List<LineupBatter> lineup = match
                        .getHomeTeam()
                        .getId()
                        .equals(teamId)
                    ...
                    return (lineup != null)
                        ? lineup
                        : Collections.<LineupBatter>emptyList();
                } catch (Exception e) {
                    log.error(
                        "Failed to parse lineup cache for matchId: {}",
                        match.getId(),
                        e
                    );
                    return Collections.<LineupBatter>emptyList();
