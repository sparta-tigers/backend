...
public class Match extends BaseEntity {
    ...
    public void updateScore(int homeScore, int awayScore) {
        ...
        if (
            this.matchResult == MatchResult.NOT_PLAYED &&
            this.matchTime != null &&
    ...
    private void validateMatchTimeState(MatchResult targetResult) {
        if (
            ...
            if (
                targetResult != MatchResult.CANCEL &&
                targetResult != MatchResult.NOT_PLAYED
            ) {
                throw new IllegalStateException(
                    "경기 예정 시간 이전에는 CANCEL 또는 NOT_PLAYED 상태만 가질 수 있습니다. (요청 상태: " +
                        targetResult +
                        ")"
                );
            }
