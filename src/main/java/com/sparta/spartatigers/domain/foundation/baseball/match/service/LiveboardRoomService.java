...
import com.sparta.spartatigers.domain.foundation.baseball.match.dto.LiveBoardRoomResponseDto;
...
import com.sparta.spartatigers.domain.foundation.baseball.match.repository.LiveBoardRoomRepository;
...
public class LiveboardRoomService {
    ...
    public String createRoomsForDay(LocalDate anyday) {
        ...
        List<Match> dayOfMatches = matchRepository.findAllByMatchTimeBetween(
            start,
            end
        );
        ...
        Set<Long> matchIds = dayOfMatches
            .stream()
            .map(match -> match.getId())
            .collect(Collectors.toSet());
        Set<Long> alreadyCreated = roomRepository
            .findAllByMatchIdIn(matchIds)
            .stream()
        ...
        long creatableCount = dayOfMatches
            .stream()
            .filter(match ->
                MatchResult.NOT_PLAYED.equals(match.getMatchResult())
            )
    ...
    public List<LiveBoardRoomResponseDto> getRoomsForDay(LocalDate anyday) {
        ...
        Map<Long, LiveBoardRoom> roomMap = roomRepository
            .findAllByDate(anyday)
            .stream()
            .collect(
                Collectors.toMap(item -> item.getMatchId(), Function.identity())
            );

        List<LiveBoardRoomResponseDto> roomDtosForDay = dayOfMatches
            .stream()
            .map(match -> {
                ...
                if (matchDate.isEqual(realToday)) {
                    // 당일 경기
                    long connectCount = connectionRepository.getConnectionCount(
                        room.getRoomId()
                    );
                    ...
                    if (stadium == null) {
                        log.warn(
                            "[ROOM] Stadium info missing for Match ID: {}.",
                            match.getId()
                        );
                        LiveBoardData liveData =
                            liveBoardDataService.getLiveBoardData(
                                match.getId()
                            );
                        InningTexts inningTexts = (liveData != null)
                            ? liveData.getInningTexts()
                            : null;
                        return LiveBoardRoomResponseDto.fromTodayMatch(
                            match,
                            room,
                            connectCount,
                            inningTexts,
                    ...
                    LiveBoardData liveData =
                        liveBoardDataService.getLiveBoardData(match.getId());
                    InningTexts inningTexts = (liveData != null)
                        ? liveData.getInningTexts()
                        : null;
    ...
    public String deleteRoomsForDay(LocalDate anyday) {
        // 1. 삭제할 날짜의 Room 선택
        String day = anyday.format(DateTimeFormatter.ofPattern("MM/dd"));
        List<LiveBoardRoom> roomsToDelete = roomRepository.findAllByDate(
            anyday
        );
        ...
        Set<Long> matchIds = roomsToDelete
            .stream()
            .map(item -> item.getMatchId())
            .collect(Collectors.toSet());
        List<Match> matches = matchRepository.findAllByIdIn(matchIds);
        Map<Long, Match> matchMap = matches
            .stream()
            .collect(
                Collectors.toMap(match -> match.getId(), Function.identity())
            );
    ...
    public LiveBoardRoomResponseDto getRoomByMatchId(Long matchId) {
        ...
        if (matchDate.isEqual(realToday)) {
            long connectCount = (room != null)
                ? connectionRepository.getConnectionCount(room.getRoomId())
                : 0L;
            // Stadium stadium = match.getStadium();

            LiveBoardData liveData = liveBoardDataService.getLiveBoardData(
                matchId
            );
            InningTexts inningTexts = (liveData != null)
                ? liveData.getInningTexts()
            ...
            if (room == null) {
                ...
                return LiveBoardRoomResponseDto.fromPastMatch(
                    match,
                    LiveBoardRoom.of(
                        roomId,
                        matchId,
    ...
    private LiveBoardRoom createRoom(Match match) {
        ...
        LiveBoardRoom newRoom = LiveBoardRoom.of(
            roomId,
            match.getId(),
            title,
            matchTime
