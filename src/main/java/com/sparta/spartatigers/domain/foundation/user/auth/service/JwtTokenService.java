...
public class JwtTokenService implements TokenService {
    ...
    public Token generateToken(TokenClaim tokenClaim) {
        ...
        Date accessTokenExpireAt = new Date(
            now + jwtConfig.getAccessToken().expire()
        );
        SecretKey accessTokenSecretKey = Keys.hmacShaKeyFor(
            jwtConfig.getAccessToken().secret().getBytes()
        );
        // refreshToken 발급 준비
        Date refreshTokenExpireAt = new Date(
            now + jwtConfig.getRefreshToken().expire()
        );
        SecretKey refreshTokenSecret = Keys.hmacShaKeyFor(
            jwtConfig.getRefreshToken().secret().getBytes()
        );
        ...
        long ttlSeconds = TimeUnit.MILLISECONDS.toSeconds(
            jwtConfig.getRefreshToken().expire()
        );
        log.info(
            "refresh expire raw={}, ttlSeconds={}",
            jwtConfig.getRefreshToken().expire(),
            ttlSeconds
        );
    ...
    public TokenClaim parseAccessToken(final String token) {
        try {
            ...
            final Number userId = claimsJws
                .getPayload()
                .get("userId", Number.class);
            final String email = claimsJws
                .getPayload()
                .get("email", String.class);
            final String nickname = claimsJws
                .getPayload()
                .get("nickname", String.class);
            final String profileImageUrl = claimsJws
                .getPayload()
                .get("profileImageUrl", String.class);
            final String userRole = claimsJws
                .getPayload()
    ...
    public TokenClaim parseRefreshToken(String refreshToken) {
        try {
            ...
            return TokenClaim.builder().subject(subject).build();
        } catch (JwtException | IllegalArgumentException e) {
            log.warn(
                "Refresh 토큰 파싱 실패 [{}]: {}",
                e.getClass().getSimpleName(),
