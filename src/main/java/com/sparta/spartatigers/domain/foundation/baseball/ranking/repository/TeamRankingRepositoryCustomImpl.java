...
public class TeamRankingRepositoryCustomImpl
    implements TeamRankingRepositoryCustom
{
    ...
    @Override
    public List<TeamRankingStat> applyTeamRecords(
        ...
        // 홈+원정 데이터 각각 집계
        List<TeamRankingStat> homeAgg = aggregateHome(
            leagueType,
            currentSeason,
            to
        );
        List<TeamRankingStat> awayAgg = aggregateAway(
            leagueType,
            currentSeason,
            to
        );
    ...
    @Override
    public List<TeamRankingStat> applyTeamRecordsByYear(
        int year,
        LeagueType leagueType
    ) {
        LocalDateTime endOfyear = Year.of(year)
            .atMonth(12)
            .atEndOfMonth()
            .atTime(23, 59, 59);
    ...
    public List<Match> findAllPostSeasonMatches(int year) {
        return queryFactory
            ...
            .fetchJoin()
            .leftJoin(match.stadium, stadium)
            .fetchJoin()
            .where(
    ...
    private void mergeInto(
        Map<Long, TeamRankingStat> rankingStatMap,
        List<TeamRankingStat> rowStats
    ) {
        for (TeamRankingStat stat : rowStats) {
            TeamRankingStat alreadyApplied = rankingStatMap.get(
                stat.getTeamId()
            );
    ...
    private List<TeamRankingStat> aggregateHome(
        LeagueType leagueType,
        int seasonYear,
        LocalDateTime anyday
    ) {
        return queryFactory
            .select(
                Projections.constructor(
                    ...
                    homeTeam.code,
                    new CaseBuilder()
                        .when(match.matchResult.eq(MatchResult.HOME_WIN))
                        .then(1)
                        .otherwise(0)
                        .sum(),
                    new CaseBuilder()
                        .when(match.matchResult.eq(MatchResult.AWAY_WIN))
                        .then(1)
                        .otherwise(0)
                        .sum(),
                    new CaseBuilder()
                        .when(match.matchResult.eq(MatchResult.DRAW))
                        .then(1)
                        .otherwise(0)
                        .sum()
                )
    ...
    private List<TeamRankingStat> aggregateAway(
        LeagueType leagueType,
        int seasonYear,
        LocalDateTime anyday
    ) {
